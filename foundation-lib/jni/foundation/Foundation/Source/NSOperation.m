...
static pthread_key_t OperationQueueKey;
static void NSOperationQueueDispose(void *queue)
{
    [(id)queue release];
}
...
{
    empty = [NSArray new];
    pthread_key_create(&OperationQueueKey, NSOperationQueueDispose);
}
...
+ (id) currentQueue
{
    return (id)pthread_getspecific(OperationQueueKey);
}
...
+ (void) _mainQueue
{
  if (mainQueue == nil)
    {
      mainQueue = [self new];
        pthread_setspecific(OperationQueueKey, [mainQueue retain]);
    }
}
...
- (id)result
{
    ...
    {
        DEBUG_LOG("NSInvocationOperation undefined results!");
        return NULL;
    }
}
@end
