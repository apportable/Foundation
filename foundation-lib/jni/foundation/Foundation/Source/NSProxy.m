...
OBJC_EXPORT id _objc_rootAllocWithZone(Class cls, void *zone);
...
/**
 * Allocates and returns an NSProxy instance in the specified zone.
 */
...
+ (IMP)instanceMethodForSelector:(SEL)aSelector
{
    ...
    /*
     *	Since 'self' is an class, class_getMethodImplementation() will get the
     *instance method.
     */
...
{
    if (class_getMethodImplementation(self, aSelector)) {
        return YES;
    }
    else{
        return NO;
...
- (id)replacementObjectForPortCoder:(NSPortCoder*)aCoder
{
    ...
    if (proxyImp == 0)
    {
        ...
        /*
         * use class_getMethodImplementation() because NSDistantObject doesn't
         *implement
         * methodForSelector:
         */
        proxyImp = class_getMethodImplementation(object_getClass((id)proxyClass), @selector(proxyWithLocal:connection:));
    }
