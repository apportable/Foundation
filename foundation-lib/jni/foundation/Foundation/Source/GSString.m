...
// Fake out the linker to bind __CFConstantStringClassReference to
// NSConstantString's class_t
extern void *__CFConstantStringClassReference __attribute__((weak, alias ("OBJC_CLASS_$_NSConstantString"), visibility("default")));
...
setup(BOOL rerun)
{
    ...
    if (!beenHere || rerun)
    {
        ...
        equalImp = (BOOL (*)(id, SEL, id))[NSStringClass instanceMethodForSelector : equalSel];
        hashSel = @selector(hash);
        hashImp = (NSUInteger (*)(id, SEL))[GSStringClass instanceMethodForSelector : hashSel];
        /*
    ...
    encoding:(NSStringEncoding)encoding
{
    ...
    if (length > 0)
    {
        ...
        else
        {
             ...
             * new memory, so we can just pass that to the designated
             ***initialiser
             */
            chars = (void*)bytes;
        }
    ...
    arguments:(va_list)argList
{
     ...
     */
    size_t sz = class_getInstanceSize(GSMutableStringClass);
    f = (GSStr)alloca(sz);
    bzero(f, sz);
...
isEqual_c(GSStr self, id anObject)
{
    ...
    else if (c == GSMutableStringClass || GSObjCIsKindOf(c, GSStringClass) == YES)
    {
        ...
        if (self->_flags.hash == 0) {
            self->_flags.hash = [self hash];
        }
        if (other->_flags.hash == 0) {
            other->_flags.hash = [other hash];
        }
...
transmute(GSStr self, NSString *aString)
{
    ...
    else
    {
                 ...
                 && [aString canBeConvertedToEncoding:internalEncoding] == YES)
        {
             ...
             * of the other string will not return data in the internal
             ***encoding.
             */
            transmute = NO;
            other = 0;
...
- (NSUInteger)hash
{
    if (self->_flags.hash == 0)
    {
        ...
        if (len > 0)
        {
            ...
            /*
             * The hash caching in our concrete string classes uses zero to
             ***denote
...
- (void)dealloc
{
    if (_contents.c != 0)
    {
        NSAssert(_flags.owned == 1, NSInternalInconsistencyException);
...
- (id)makeImmutableCopyOnFail:(BOOL)force
{
    ...
    if (_flags.wide == 1)
    {
        GSPrivateSwizzle(self, [GSUnicodeBufferString class]);
    }
    else
    {
        GSPrivateSwizzle(self, [GSCBufferString class]);
    }
#ifndef NDEBUG
