/**
   ...
   $Date: 2010-09-15 19:55:24 -0700 (Wed, 15 Sep 2010) $ $Revision: 31355 $
 */
    ...
    if ([aCoder allowsKeyedCoding])
    {
        ...
        if (length > 0)
        {
            ...
            else
            {
                NSUInteger i = 0;
                NSUInteger pos = 0;
                NSMutableArray *rangeAttributes = [NSMutableArray arrayWithCapacity:1];
                NSMutableData *info = [NSMutableData dataWithCapacity:2];

                while (pos < length)
                {
                    [rangeAttributes addObject:[self attributesAtIndex:pos
                                                effectiveRange:&range]];
                    appendUIntData(info, range.length);
                    appendUIntData(info, i++);
                    pos = NSMaxRange(range);
                }
                [aCoder encodeObject:[[rangeAttributes copy] autorelease]
                 forKey:@"NSAttributes"];
    ...
    inRange:(NSRange)rangeLimit
{
    ...
    if (rangeLimit.location == NSNotFound || NSMaxRange(rangeLimit) > [self length])
    {
...
VERDE_UNIMPLEMENTED_VOID_FN(-(void)enumerateAttributesInRange:(NSRange)range
                            options:(NSAttributedStringEnumerationOptions)options
                            usingBlock:(void (^)(NSDictionary *attrs, NSRange range, BOOL *stop))block);

VERDE_UNIMPLEMENTED_VOID_FN(-(void)enumerateAttribute:(NSString *)name
...
- (void)addAttribute:(NSString*)name value:(id)value range:(NSRange)aRange
{
    ...
    if (effectiveRange.location < NSMaxRange(aRange))
    {
        ...
        while (effectiveRange.location < NSMaxRange(aRange))
        {
            ...
            {
                effectiveRange.location = NSMaxRange(aRange); // stop the
                                                              // loop...
            }
