/** Implementation for NSProcessInfo for GNUStep
   ...
   $Date: 2010-06-09 08:19:01 -0700 (Wed, 09 Jun 2010) $ $Revision: 30636 $
 */
...
#undef __MINGW__  // This is one place where we don't want MinGW
...
+ (void)load
{
     ...
     * Second, to know how long each argument is, and allocate memory
     *accordingly.
...
+ (void)initialize
{
        ...
        && !_gnu_processName && !_gnu_arguments && !_gnu_environment)
    {
        ...
        {
            _gnu_process_args(*_NSGetArgc(), *_NSGetArgv(), NULL);
            return;
        }
