        ...
        if (sendPort != nil)
        {
            ...
            else if (![recvPort isMemberOfClass:[sendPort class]])
            {
                /*
                   ...
                   connections using the same port class. For other port
                      classes,
    ...
    sendPort:(NSPort*)s
{
    ...
    /*
     * This maps request sequence numbers to the NSPortCoder objects
     *representing
    ...
    NS_DURING
    {
        ...
        if (out_parameters)
        {
                 ...
                 etmptype = skip_argspec (etmptype))
            {
                /* Get the type qualifiers, like IN, OUT, INOUT, ONEWAY. */
                flags = objc_get_type_qualifiers(etmptype);
                /* Skip over the type qualifiers, so now TYPE is pointing
                   directly
                   at the char corresponding to the argument's type, as defined
                   in <objc/objc-api.h> */
                tmptype = objc_skip_type_qualifiers (tmptype);
