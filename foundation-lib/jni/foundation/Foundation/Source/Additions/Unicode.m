...
tables:
    ...
    *size = dpos;
    ...
    else if (ptr != buf && !((ptr + dpos) >= buf && (ptr + dpos) <= (buf + BUFSIZ))
             && (dst == 0 || ptr != *dst))
    {
        // Simple (ptr != buf) check sometimes is not enough, we have to check
        // that (ptr + dpos) is not inside buf, see GROW macro.
        NSZoneFree(zone, ptr);
    }
              ...
              unsigned int options)
{
    ...
    case NSMacOSRomanStringEncoding:
        DEBUG_LOG("Warning: Operations on NSMacOSRomanStringEncoding are unsupported. Falling back to NSUTF8StringEncoding. Expect errors!");
    case NSUTF8StringEncoding:
    {
        if (swapped == YES)
        {
...
bases:
        ...
        {
            /* Code below operates under assumption that 'ptr' always has enough
             * space. But that is not true if 'dst' is NULL - in this case 'ptr'
             * points to stack buffer of size 'bsize'. So instead of overwriting
             * stack we just fail. This needs to be fixed of course.
             */
            result = NO;
            goto done;
        }
                ...
                if (rval != 0)
                {
                    if (rval == (size_t)-1)
                    {
                        ...
                        else if (errno == EILSEQ)
                        {
                            ...
                            /*
                             * If we are allowing lossy conversion, we replace
                             * any
        ...
        if (natEnc == GSUndefinedEncoding)
        {
            ...
            if (strcmp(encoding, "ANSI_X3.4-1968") == 0 /* glibc */
                ...
                natEnc = NSBIG5StringEncoding;
                     ...
                     || strcmp(encoding, "eucKR") == 0 /* HP-UX IRIX OSF/1
                                                          NetBSD */
