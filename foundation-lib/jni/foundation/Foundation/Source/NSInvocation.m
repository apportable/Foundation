/** Implementation of NSInvocation for GNUStep
   ...
   $Date: 2012-01-29 09:57:06 -0700 (Sun, 29 Jan 2012) $ $Revision: 34660 $
 */
...
- (id)initWithSize:(NSUInteger)_size
{
    ...
    if (self)
    {
        ...
        buffer = mmap(NULL, PAGE_SIZE, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_ANON|MAP_SHARED, -1, 0);
        if (buffer == (void*)-1)
        {
            buffer = (void*)0;
        }
        ...
        {
            bzero(buffer, _size);
            buffer = (void*)(((uintptr_t)buffer+3) & ~3);
        }
        ...
        if (buffer == (void*)0)
        {
            ...
            DEBUG_LOG("Failed to map %d bytes for execute: %s", _size, [[[NSError _last] description] UTF8String]);
            buffer = 0;
            executable = 0;
            [self dealloc];
            self = nil;
...
{
#ifdef ANDROID
    cacheflush((long)buffer, (long)buffer + NSPageSize(), 0);
#endif
#if !defined(HAVE_FFI_PREP_CLOSURE_LOC)
