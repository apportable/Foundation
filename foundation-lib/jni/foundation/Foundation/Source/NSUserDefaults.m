/** Implementation for NSUserDefaults for GNUstep
   ...
   $Date: 2010-06-19 03:48:21 -0700 (Sat, 19 Jun 2010) $ $Revision: 30784 $
 */
...
+ (void)resetStandardUserDefaults
{
    NS_DURING
    {
        if (sharedDefaults != nil)
        {
            ...
            /* To ensure that we don't try to synchronise the old defaults to
               disk
            ...
            /* Ensure changes are written, and no changes left so we can't end
               up
...
+ (NSUserDefaults*)standardUserDefaults
{
    ...
    /*
     * Calling +standardUserDefaults and +userLanguages is horribly
     *interrelated.
     ...
     * +standardUserDefaults then sets up language domains and loads
     *localisation
     ...
     * We need to lock and check again before CREATING sharedDefaults if it
     *doesn't exist,
     * so that two threads can't create it at once (or call
     *resetStandardUserDefaults at
     * the same time).
     * By not locking here, we avoid a deadlock that can occur between classLock
     *and _lock. */
    ...
    NS_DURING
    {
         ...
         *
         * We iterate over <language>, and for each <language> we iterate over
         *GNUSTEP_LIBRARY.
         */
...
stringByAppendingPathComponent: @"gnustep-base"]
            ...
            while ((lang = [enumerator nextObject]))
            {
                ...
                {
                    path = [path stringByAppendingPathComponent:@"Defaults.plist"];
                ...
                else
                {
                    ...
                    [dict writeToFile:[[paths lastObject] stringByAppendingPathComponent:@"Defaults.plist"] atomically:NO];
                }
                if (dict != nil)
                {
    ...
    [sharedDefaults setObject :[NSArray arrayWithObject:[[NSLocale preferredLanguages] objectAtIndex:0]] forKey : @"AppleLanguages"];
    return AUTORELEASE(sharedDefaults);
}
...
- (id)initWithContentsOfFile:(NSString*)path
{
    ...
    // Create volatile defaults and add the Argument and the Registration
    // domains
...
- (void)removeObjectForKey:(NSString*)defaultName
{
    ...
    NS_DURING
    {
        ...
        if ([(NSDictionary*)obj objectForKey : defaultName] != nil)
        {
...
{
    NSNumber  *n = [NSNumberClass numberWithLongLong:value];
    ...
    NS_DURING
    {
        ...
        while (done == NO)
        {
            if ([key hasPrefix:@"-"] == YES && [key isEqual:@"-"] == NO)
            {
                ...
                /* anything beginning with a '-' is a defaults key and we must
                   strip
                    the '-' from it.  As a special case, we leave the '- in
                       place
