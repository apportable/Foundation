...
typedef struct {
    ...
    id <NSURLProtocolClient>  client;   // Retained, see Apple docs
...
- (void)dealloc
{
    if (this != 0)
    {
        ...
        if (this->input != nil)
        {
            ...
            [this->input removeFromRunLoop:this->_runloop forMode:this->_mode];
            [this->output removeFromRunLoop:this->_runloop forMode:this->_mode];
            [this->input close];
            [this->output close];
            DESTROY(this->input);
            DESTROY(this->output);
...
- (id)init
{
    if ((self = [super init]) != nil)
    {
        if (object_getClass(self) != abstractClass && object_getClass(self) != placeholderClass)
        {
            _NSURLProtocolInternal = NSZoneCalloc([self zone], 1, sizeof(Internal));
        }
    }
    return self;
}
...
- (void)timeout
{
    [self stopLoading];
    if (this->client != NULL) {
        [this->client URLProtocol:self didFailWithError:[NSError errorWithDomain:@"timeout" code:0 userInfo:NULL]];
        [this->client release];
        this->client = NULL;
    }
}
...
- (void)start
{
    // This is wrong - timeout should specify maximum time frame a single
    // read/write operation
    // can take, and not the total time. But we don't have time to investigate
    // this properly, so
    // lets just scale!
    [self performSelector:@selector(timeout) withObject:NULL afterDelay:[[self request] timeoutInterval] * 5];
}
...
- (void)startLoading
{
    [self startLoading:[NSRunLoop currentRunLoop] forMode:NSDefaultRunLoopMode];
}
...
- (void)startLoading:(NSRunLoop *)rl forMode:(NSString *)mode
{
    ...
    else
    {
        ...
        {
            [self stopLoading];
            [this->client URLProtocol:self didFailWithError:[NSError errorWithDomain:@"can't resolve host" code:0 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:url, @"NSErrorFailingURLKey", @"can't resolve host", @"NSLocalizedDescription", nil]]];
            [self finish];
            return;
        }
...
{
    if(this->decompressing != 0) {
        NSMutableData *inData = nil;
        if(this->compressed != nil) {
            inData = [[this->compressed mutableCopy] autorelease];
            [inData appendData:d];
        } else {
            inData = [[d mutableCopy] autorelease];
        }

        this->z.next_in = inData.bytes;
        this->z.avail_in = inData.length;

        size_t dataBlockSize = inData.length * 2;
        size_t currentCompressedOutSize = dataBlockSize;
        Bytef *decompressedOut = malloc(dataBlockSize * sizeof(Bytef));
        this->z.next_out = decompressedOut;
        this->z.avail_out = currentCompressedOutSize;
        this->z.total_out = 0;
        int returnValue = inflate(&this->z, Z_SYNC_FLUSH);
        while(returnValue >= 0) {
            if(returnValue == Z_STREAM_END) {
                break;
            }

            if(returnValue == Z_OK && this->z.avail_out == 0) {
                currentCompressedOutSize += dataBlockSize;
                decompressedOut = realloc(decompressedOut, currentCompressedOutSize);
                this->z.next_out = decompressedOut + this->z.total_out;
                this->z.avail_out += dataBlockSize;
                returnValue = inflate(&this->z, Z_SYNC_FLUSH);
            } else {
                break;
            }
        }
        ...
        if(returnValue >= 0) {
            ...
            this->compressed = nil;
            [this->client URLProtocol:self didLoadData:[NSData dataWithBytes:decompressedOut length:this->z.total_out]];
        } else if(returnValue == Z_BUF_ERROR) {
            [this->compressed release];
            this->compressed = [[NSData alloc] initWithBytes:(inData.bytes + this->z.total_in + 1)
                                length:(inData.length - this->z.total_in)];
            [this->client URLProtocol:self didLoadData:[NSData dataWithBytes:decompressedOut length:this->z.total_out]];
        } else {
            DEBUG_LOG("Decompression failed: %s", this->z.msg);
            DEBUG_BREAK();
        }
        free(decompressedOut);
    } else {
        [this->client URLProtocol:self didLoadData:d];
    }
}

- (BOOL)_isDocumentFinished:(GSMimeDocument *)document length:(NSInteger)bodyLength
{
    NSString *s = [[document headerNamed:@"content-length"] value];
    if ([s length] > 0)
    {
        if ([s intValue] <= bodyLength)
        {
            return YES;
        }
    }
    return NO; // really is maybe...
}

- (void)_got:(NSStream*)stream
{
    [[stream retain] autorelease];  // We're re-entrant and it crashes accessing
                                    // stream on errors if we don't do this
...
maxLength: sizeof(buffer)];
    if (readCount < 0)
    {
        ...
        {
            e = [[[stream streamError] retain] autorelease];
            if (_debug)
            {
    ...
    else
    {
        ...
        if (wasInHeaders == YES && isInHeaders == NO)
        {
            ...
            if ([s isEqualToString:@"gzip"] || [s isEqualToString:@"x-gzip"] || [s isEqualToString:@"deflate"])
            {
                ...
                this->z.avail_in = 0;
                int inflateResult = inflateInit2(&this->z, 15+32);
                if(inflateResult != Z_OK) {
                    switch(inflateResult) {
                    case Z_MEM_ERROR:
                        DEBUG_LOG("Coupld not decompress HTTP response because there was no more memory");
                        break;
                    case Z_VERSION_ERROR:
                        DEBUG_LOG("HTTP response not decompressed because of zlib version error");
                        break;
                    case Z_STREAM_ERROR:
                        DEBUG_LOG("HTTP response not decompressed because of zlib stream error: %s", this->z.msg);
                        break;
                    default:
                        DEBUG_LOG("Could not decompress HTTP response");
                    }
                    this->decompressing = NO;
                }
            }
#endif
        }
            ...
            if (_isLoading == YES)
            {
                ...
                if (bodyLength > _parseOffset)
                {
                    if (_parseOffset > 0)
                    {
                        ...
                        d = [NSData dataWithData:d];  // Ensure that it's not
                                                      // mutable
                    }
                 ...
                 */
                if (_isLoading == YES || [self _isDocumentFinished:[_parser mimeDocument] length:bodyLength])
                {
                    _isLoading = NO;
        ...
        else if (_isLoading == YES && _statusCode != 401)
        {
            ...
            if ([_parser isInBody])
            {
                ...
                if ([self _isDocumentFinished:[_parser mimeDocument] length:bodyLength])
                {
                    _isLoading = NO;
                    [this->client URLProtocolDidFinishLoading:self];
                    [self finish];
...
- (void)stream:(NSStream*)stream handleEvent:(NSStreamEvent)event
{
    ...
    else if (stream == this->output)
    {
        ...
        case NSStreamEventHasSpaceAvailable:
        {
            ...
            {
                // If the _body stream doesn't respond to hasBytesAvailable, go
                // ahead and assume
                // that the answer is yes.  Otherwise, ask nicely.
                if (![_body respondsToSelector:@selector(hasBytesAvailable)] || [_body hasBytesAvailable])
                {
                    unsigned char buffer[BUFSIZ*64];
                    int len;
            ...
            if (written < 0 && [this->output streamStatus] == NSStreamStatusError)
            {
                NSError* e = [[[this->output streamError] retain] autorelease];
                if (_debug)
                {
                    NSLog(@"%@ send error %@", self, e);
                }
                [self stopLoading];
                [this->client URLProtocol:self didFailWithError:e];
                [self finish];
...
//			[this->client URLProtocol: self didLoadData: [NSData
// dataWithBytes: buffer length: len]];	// notify
            return;
        }
        case NSStreamEventEndEncountered:   // can this occur in parallel to
