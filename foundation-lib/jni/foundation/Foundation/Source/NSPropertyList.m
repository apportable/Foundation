        ...
        if (pld->pos < pld->end && pld->ptr[pld->pos] == '*')
        {
            ...
            if (len > 1)
            {
                ...
                else if (type == 'D')
                {
                    ...
                    for (unsigned idx = 0; idx < len; idx++) buf[idx] = ptr[idx];
            ...
            while (stride > (STRIDE_FACTOR - 1))
            {
                ...
                for (c = stride; c < count; c++)
                {
                    ...
                    while (!found)
                    {
                        ...
                        id b = keys[d];
                        Class cls;
                        NSComparisonResult r = 0;

                        cls = object_getClass(a);
                        if (cls != lastClass)
                        {
                            lastClass = cls;
    ...
    error:(NSError**)error
{
    ...
    return [self dataFromPropertyList:propertyList format:format errorDescription:nil];
}
...
- (id)objectAtIndex:(NSUInteger)index
{
    ...
    {
        unsigned char uidIdx;

        [data getBytes:&uidIdx range:NSMakeRange(counter,1)];
        result = [NSDictionary dictionaryWithObject:
                  [NSNumber numberWithInt:uidIdx]
                  forKey:@"CF$UID"];
    }
    else if (next == 0x81)
    {
        unsigned short uidIdx;

        [data getBytes:&uidIdx range:NSMakeRange(counter,2)];
        uidIdx = NSSwapBigShortToHost(uidIdx);
        result = [NSDictionary dictionaryWithObject:
                  [NSNumber numberWithInt:uidIdx]
                  forKey:@"CF$UID"];
    }
...
- (void)storeString:(NSString*)string
{
    ...
    else
    {
        if (len < 0x0F)
        {
            ...
            {
                buffer[idx] = NSSwapHostShortToBig(buffer[idx]);
            }
