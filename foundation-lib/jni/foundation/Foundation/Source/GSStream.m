...
// For the CFStream implementations these really should be the same pointers but
// we can
// settle for atleast the same value as constants (which should get optimized to
// the same pointer
...
NSString * const NSStreamSOCKSProxyConfigurationKey = @"kCFProxyAutoConfigurationURLKey";
NSString * const NSStreamSOCKSProxyHostKey = @"SOCKSProxy";
NSString * const NSStreamSOCKSProxyPasswordKey = @"kCFProxyPasswordKey";
NSString * const NSStreamSOCKSProxyPortKey = @"SOCKSPort";
NSString * const NSStreamSOCKSProxyUserKey = @"kCFProxyUsernameKey";
...
{
    if (_loops && aRunLoop != nil && mode != nil)
    {
...
{
    // A null check is neccessary since this may be the CF counterparts
    // implementation
    if (property == NULL)
    {
        [[[_properties objectForKey:key] retain] autorelease];
...
- (void)_sendEvent:(NSStreamEvent)event
{
    [self _sendEvent:event toDelegate:(_delegateValid ? _delegate : nil)];
}
...
- (void)_sendEvent:(NSStreamEvent)event toDelegate:(id)delegate
{
    ...
    else if (event == NSStreamEventHasSpaceAvailable)
    {
        ...
        if ((_events & NSStreamEventHasSpaceAvailable) == 0)
        {
            ...
            [delegate stream:self handleEvent:NSStreamEventHasSpaceAvailable];
        }
    }
