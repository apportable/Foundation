/** Implementation for NSCalendarDate for GNUstep
   ...
   $Date: 2010-08-06 22:02:28 -0700 (Fri, 06 Aug 2010) $ $Revision: 31087 $
 */
...
// Ensure the defines are NSTimeIntervals aka doubles
#undef USEC_PER_SEC
...
// Used to store the base of time adjustments
static NSTimeInterval time_base = 0.0;

static void initialize_clock() __attribute__((constructor));
static void initialize_clock()
{
    struct timeval monotonic_base;
    gettimeofday(&monotonic_base, NULL);
    struct timespec time;
    clock_t result = clock_gettime(CLOCK_MONOTONIC, &time);
    time_base = (NSTimeInterval)monotonic_base.tv_sec - NSTimeIntervalSince1970;
    time_base += (NSTimeInterval)monotonic_base.tv_usec / USEC_PER_SEC;
    // adjust the time_base according to the time interval, since the MONOTONIC
    // clock is based on the time interval since boot
    time_base -= (NSTimeInterval)time.tv_sec;
    time_base -= time.tv_nsec / NSEC_PER_SEC;
}
...
GSTimeNow(void)
{
    ...
    struct timespec time;
    clock_t result = clock_gettime(CLOCK_MONOTONIC, &time);
    t = (NSTimeInterval)time.tv_sec + time.tv_nsec / NSEC_PER_SEC + time_base;
#else
    struct timeval tp;

    gettimeofday (&tp, NULL);
    t = (NSTimeInterval)tp.tv_sec - NSTimeIntervalSince1970;
    t += (NSTimeInterval)tp.tv_usec / USEC_PER_SEC;
#if 1
                ...
                case 'Z':
                    ...
                    sourceIdx += tmpIdx;
                    {
                        ...
                        /* Abbreviations aren't one-to-one with time zone names
                           so just look for the zone named after the
                              abbreviation,
    ...
    info:(DescriptionInfo*)info
{
    ...
    while (i < lf)
    {
        ...
        if (f[i] == '%')
        {
            ...
            while (fmtlen < 5 && f[i] >= '0' && f[i] <= '9') // field width
                                                             // specified
            {
                fldfmt[fmtlen++] = f[i];
            ...
            {
                // ignore formats that specify field width greater than the max
                // allowed
                i -= fmtlen; // set i back so all ignored characters will be
                             // copied
            }
            ...
            case 'y':
                ...
                if (ycent)
                {
                    if (fmtlen == 1) // no format width specified; supply
                                     // default
                    {
                        fldfmt[fmtlen++] = '0';
                        fldfmt[fmtlen++] = '4';
