...
- (void)encodeWithCoder:(NSCoder*)encoder
{
    ...
    [encoder encodeValueOfObjCType:@encode(BOOL) at:&_alwaysShowDecimalSeparator];
    ...
    errorDescription:(NSString**)error
{
    ...
    DEBUG_LOG("NSNumberFormatter-getObjectValue:forString:... not fully implemented");
...
- (id)init
{
    ...
    if (self) {
        ...
        _decimalSeparator = [[[NSLocale currentLocale] objectForKey:NSDecimalSeparator] characterAtIndex:0];
        _thousandSeparator = [[[NSLocale currentLocale] objectForKey:NSThousandsSeparator] characterAtIndex:0];
        _alwaysShowDecimalSeparator = NO;

        _hasThousandSeparators = YES;

        NSAttributedString *nilString = [[NSAttributedString alloc] initWithString:@""];
        [self setAttributedStringForNil:nilString];
        [nilString release];

        NSAttributedString *nanString = [[NSAttributedString alloc] initWithString:@"NaN"];
        [self setAttributedStringForNotANumber:nanString];
        [nanString release];
    }
...
- (id)initWithCoder:(NSCoder*)decoder
{
    ...
    else
    {
        ...
        [decoder decodeValueOfObjCType:@encode(BOOL) at:&_alwaysShowDecimalSeparator];
...
- (NSString*)stringForObjectValue:(id)anObject
{
    ...
    if (_localizesFormat)
    {
        ...
        defaultDecimalSeparator = [defaultLocale objectForKey:NSDecimalSeparator];
        defaultThousandsSeparator = [defaultLocale objectForKey:NSThousandsSeparator];
    }
    ...
    formattingCharacters = [NSCharacterSet characterSetWithCharactersInString:@"0123456789#.,_"];
    placeHolders = [NSCharacterSet characterSetWithCharactersInString:@"0123456789#_"];
    ...
    NSDecimal decimalValue = [anObject decimalValue];
    if (NSDecimalIsNotANumber(&decimalValue))
    {
        return [[self attributedStringForNotANumber] string];
    }
    if (_attributedStringForZero && [anObject isEqual:[NSDecimalNumber zero]])
    {
        return [[self attributedStringForZero] string];
    }

    useFormat = _positiveFormat;
    if ([(NSNumber*)anObject compare :[NSDecimalNumber zero]] == NSOrderedAscending)
    {
    ...
    if (nil == useFormat)
    {
        useFormat = [NSString stringWithFormat:@"%@#%@###%@##", negativeNumber ? @"-":@"", defaultThousandsSeparator, defaultDecimalSeparator];
    }
    ...
    if ([self hasThousandSeparators] && (0 != [useFormat rangeOfString:defaultThousandsSeparator].length))
    {
        displayThousandsSeparators = YES;
    }

    if ([self allowsFloats] && (NSNotFound != [useFormat rangeOfString:defaultDecimalSeparator].location))
    {
        decimalPlaceRange = [useFormat rangeOfString:defaultDecimalSeparator options:NSBackwardsSearch];
        ...
        {
            while ([placeHolders characterIsMember:[useFormat characterAtIndex:NSMaxRange(decimalPlaceRange)]])
            {
        ...
        if (0 != decimalPlaces || _alwaysShowDecimalSeparator)
        {
    ...
    NSDecimalRound(&roundedDecimal, &representativeDecimal, decimalPlaces, NSRoundPlain);
    roundedNumber = [NSDecimalNumber decimalNumberWithDecimal:roundedDecimal];
#else
    // Apportable prefers arbitrary precision instead of rounding
    roundedNumber = [NSDecimalNumber decimalNumberWithString:[anObject stringValue]];
#endif
    ...
    if (negativeNumber)
    {
        roundedNumber = [roundedNumber decimalNumberByMultiplyingBy:(NSDecimalNumber*)[NSDecimalNumber numberWithInt:-1]];
    }
    intPart = (NSDecimalNumber*)[NSDecimalNumber numberWithInt:(int)[roundedNumber doubleValue]];
    fracPart = [roundedNumber decimalNumberBySubtracting:intPart];
    intPartString = [[[intPart descriptionWithLocale:locale] mutableCopy] autorelease];
    ...
    {
        while (([placeHolders characterIsMember:[useFormat characterAtIndex:NSMaxRange(intPartRange)]] ||
                [[useFormat substringFromRange:NSMakeRange(NSMaxRange(intPartRange), 1)] isEqual:defaultThousandsSeparator]) &&
               NSMaxRange(intPartRange) < [useFormat length] - 1)
        {
            intPartRange.length++;
        }
    }
    intPad = [[[useFormat substringWithRange:intPartRange] mutableCopy] autorelease];
    [intPad replaceOccurrencesOfString:defaultThousandsSeparator withString:@"" options:0 range:NSMakeRange(0, [intPad length])];
    [intPad replaceOccurrencesOfString:@"#" withString:@"" options:NSAnchoredSearch range:NSMakeRange(0, [intPad length])];
    if ([intPad length] > [intPartString length])
    {
        NSRange ipRange;

        ipRange = NSMakeRange(0, [intPad length] - [intPartString length] + 1);
        [intPartString insertString:[intPad substringWithRange:ipRange] atIndex:0];
        [intPartString replaceOccurrencesOfString:@"_" withString:@" " options:0 range:NSMakeRange(0, [intPartString length])];
        [intPartString replaceOccurrencesOfString:@"#" withString:@"0" options:0 range:NSMakeRange(0, [intPartString length])];
    }
    ...
    if (displayFractionalPart)
    {
        if (0 != decimalPlaces)
        {
            ...
            fracPart = [fracPart decimalNumberByMultiplyingByPowerOf10:decimalPlaces];
            ms = [[fracPart descriptionWithLocale:locale] mutableCopy];
            [ms replaceOccurrencesOfString:@"0" withString:@"" options:(NSBackwardsSearch | NSAnchoredSearch) range:NSMakeRange(0, [ms length])];
            if ([fracPad length] > [ms length])
            {
                NSRange fpRange;

                fpRange = NSMakeRange([ms length], ([fracPad length] - [ms length]));
                [ms appendString:[fracPad substringWithRange:fpRange]];
                [ms replaceOccurrencesOfString:@"#" withString:@"" options:(NSBackwardsSearch | NSAnchoredSearch) range:NSMakeRange(0, [ms length])];
                [ms replaceOccurrencesOfString:@"#" withString:@"0" options:0 range:NSMakeRange(0, [ms length])];
                [ms replaceOccurrencesOfString:@"_" withString:@" " options:0 range:NSMakeRange(0, [ms length])];
            }
...
{
    // TODO(jackson): Finish implementing NSNumberFormatter. For common cases,
...
{
    return _alwaysShowDecimalSeparator;
}

- (void)setAlwaysShowsDecimalSeparator:(BOOL)flag
{
    [self willChangeValueForKey:@"alwaysShowDecimalSeparator"];
    _alwaysShowDecimalSeparator = flag;
    [self didChangeValueForKey:@"alwaysShowDecimalSeparator"];
}

/* Configuring Numeric Symbols */

VERDE_UNIMPLEMENTED_VOID_FN(-(void)setLocale:(NSLocale *)locale);
VERDE_UNIMPLEMENTED_FN(-(NSLocale *)locale);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setPercentSymbol:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)percentSymbol);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setPerMillSymbol:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)perMillSymbol);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setMinusSign:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)minusSign);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setPlusSign:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)plusSign);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setExponentSymbol:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)exponentSymbol);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setZeroSymbol:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)zeroSymbol);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setNilSymbol:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)nilSymbol);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setNotANumberSymbol:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)notANumberSymbol);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setNegativeInfinitySymbol:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)negativeInfinitySymbol);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setPositiveInfinitySymbol:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)positiveInfinitySymbol);

/* Configuring the Format of Currency */

VERDE_UNIMPLEMENTED_VOID_FN(-(void)setCurrencySymbol:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)currencySymbol);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setCurrencyCode:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)currencyCode);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setInternationalCurrencySymbol:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)internationalCurrencySymbol);
VERDE_UNIMPLEMENTED_VOID_FN(-(void)setCurrencyGroupingSeparator:(NSString *)string);
VERDE_UNIMPLEMENTED_FN(-(NSString *)currencyGroupingSeparator);
