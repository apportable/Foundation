...
typedef struct ParserStateStruct
{
    ...
    /**
     * Error value, if this parser is currently in an error state, nil
     *otherwise.
     */
    NSError *error;
} ParserState;
...
updateStreamBuffer(ParserState* state)
{
     ...
     * If the stream doesn't do buffering, then we need to get data one
     *character
     * at a time.
     */
    if (![stream getBuffer:&buffer length:&length])
    {
        ...
        // Set the source index to -1 so it will be 0 when we've finished with
        // it
        state->sourceIndex = -1;
        state->bufferIndex = 0;
        state->bufferLength = 1;
    }
    // Use an NSString to do the character set conversion.  We could do this
    // more
...
parseError(ParserState *state)
{
    ...
    NSDictionary *userInfo = [[NSDictionary alloc] initWithObjectsAndKeys:
                              ...
                              ([NSString stringWithFormat:@"Unexpected character %c at index %d",
...
parseValue(ParserState *state)
{
    ...
    c = consumeSpace(state);
    //   2.1: A JSON value MUST be an object, array, number, or string, or one
    // of the
...
+ (NSData*)dataWithJSONObject:(id)obj
    ...
    error:(NSError **)error
{
    /* Temporary string: allocate more space than we are likely to use so we
       just
