...
- (void)_setHeaders:(id)headers
{
    ...
    else if ([headers isKindOfClass:[NSArray class]] == YES)
    {
        ...
        while ((h = [e nextObject]) != nil)
        {
            ...
            if ([n caseInsensitiveCompare:@"content-length"] == NSOrderedSame)
            {
                contentLength = v;
            }
...
- (BOOL)_appendData:(NSData *)data
{
    ...
    if(!this->complete)
    {
        ...
        //Since it is already known that this response has an incomplete header,
        // the search for the delimiter can start at the point of appending
        for(const char *msg = [this->data bytes] + pos; pos < length - 4; msg = [this->data bytes] + (++pos))
        {
            if(strncmp(msg, "\r\n\r\n", 4) == 0)
            {
                ...
                NSString *headerStr = [[NSString alloc] initWithData:[this->data subdataWithRange:NSMakeRange(0, pos)] encoding:NSISOLatin1StringEncoding]; //IIRC
                                                                                                                                                            // the
                                                                                                                                                            // encoding
                                                                                                                                                            // for
                                                                                                                                                            // http
