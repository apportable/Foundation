...
/*
   ...
   This means we have more big numbers than one would expect (almost up to
      10**165)
   ...
   [eg minus 38] or relative to the number of digits in the mantissa [minus
      length].)
...
typedef struct {
    ...
    unsigned char cMantissa[2*NSDecimalMaxDigit]; /* Make this big enough for
                                                    multiplication */
} GSDecimal;
...
GSDecimalString(const GSDecimal *number, NSDictionary *locale)
{
    ...
    // Apportable hasn't had an app that needed exponents yet, so just always
    // use normal format
    if (YES) // (number->length <= 6) && (0 < size) && (size < 7))
    {
        ...
        for (i = 0; i < number->length; i++)
        {
            if (size == i)
            {
                // Apportable likes to add a leading zero
                if (i == 0) {
                    [string appendString:@"0"];
                }
                [string appendString:sep];
                    ...
                    NSDictionary *locale)
{
    ...
    const char *s;
    int i;
    BOOL overrideValid = NO; // zero should have zero length
    ...
    if (!result->length && !overrideValid) {
        result->validNumber = NO;
    }
...
GSSimpleAdd(NSDecimal *result, const NSDecimal *left, const NSDecimal *right,
            NSRoundingMode mode)
{
    // left and right are both valid and positive, non-zero. The have been
    // normalized and
    // left is bigger than right. result, left and right all point to different
    // entities.
