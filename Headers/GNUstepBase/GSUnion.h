...
typedef	union {
  ...
  // Since nil (and Nil) is defined as ((void*)0) (which is in par with Apple)
  // we need to add void* to _OBJ unions to allow casting nil. If there is no
  // void* member in union casts like (GSIArrayItem)nil fail with:
  // "cast to union type from type 'void *' not present in union".
  void  *obj_ptr;
  id		obj;
  NSObject	*nso;
#endif
