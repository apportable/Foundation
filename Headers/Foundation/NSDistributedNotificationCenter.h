 ...
 *  process to which the notification should be delivered is suspended:
   ...
   NSNotificationSuspensionBehaviorCoalesce,   // drop all for this process but
 * the latest-sent notification
   NSNotificationSuspensionBehaviorHold,       // queue all notifications for
 * this process until it is resumed
   NSNotificationSuspensionBehaviorDeliverImmediately  // resume the process and
 * deliver
   }
   </example>
 */
