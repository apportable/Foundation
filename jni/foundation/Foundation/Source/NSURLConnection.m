...
// Why this method wasnt in GNUStep to begin with is beyond me...
- (void) connection: (NSURLConnection *)connection
...
#else
      o->_NSURLConnectionInternal = calloc(1, sizeof(Internal));
#endif
    }
  return o;
}
...
#ifdef ANDROID
        this->_protocol = [[[[NSPlatform currentPlatform] URLProtocolClass] alloc] initWithRequest:this->_request connection:self];
        [this->_protocol setAsynchronous:YES];
#else
			      ...
			      error: (NSError **)error
{
    ...
    if ([self canHandleRequest: request] == YES)
    {
        if (response)
            *response = NULL;
        _NSURLConnectionDataCollector *collector = [[_NSURLConnectionDataCollector alloc] initWithResponsePointer:response andErrorPointer:error];
        NSURLConnection *o = [[self alloc] init];
        inst->_request = [request copy];
        inst->_delegate = AUTORELEASE(collector);
        inst->_protocol = [[[[NSPlatform currentPlatform] URLProtocolClass] alloc] initWithRequest:inst->_request connection:o];
        [inst->_protocol setAsynchronous:NO];
        [inst->_protocol startLoading];
        inst->_debug = GSDebugSet(@"NSURLConnection");
