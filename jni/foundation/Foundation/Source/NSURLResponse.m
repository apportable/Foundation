...
- (BOOL)_appendData:(NSData *)data
{
    size_t pos = [this->data length];
    [this->data appendData:data];
    if(!this->complete)
    {
        NSUInteger length = [this->data length];
        //Since it is already known that this response has an incomplete header, the search for the delimiter can start at the point of appending
        for(const char *msg = [this->data bytes] + pos; pos < length - 4; msg = [this->data bytes] + (++pos))
        {
            if(strncmp(msg, "\r\n\r\n", 4) == 0)
            {
                this->complete = YES;
                
                NSString *headerStr = [[NSString alloc] initWithData:[this->data subdataWithRange:NSMakeRange(0, pos)] encoding:NSISOLatin1StringEncoding]; //IIRC the encoding for http headers is iso latin 1, this might not be correct
                NSArray *lines = [headerStr componentsSeperatedByString:@"\r\n\r\n"];
                if([lines count] > 0)
                {
                    this->statusText = [[lines objectAtIndex:0] copy];
                    if (this->headers == 0)
                    {
                        this->headers = [_GSMutableInsensitiveDictionary new];
                    }
                    ...
                    {
                        NSString *headerItem = [lines objectAtIndex:idx];
                        NSRange delimRange = [headerItem rangeOfString:@":"];
                        if(delimRange.location != NSNotFound)
                        {
                            NSString *key = [[headerItem substringToIndex:delimRange.location] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
                            NSString *value = [[headerItem substringFromIndex:NSMaxRange(delimRange)] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
                            [this->headers setObject:value forKey:key];
                        }
                    }
                }
                else
                {
                    this->statusText = [@"" copy];
                }
                //truncate the message headers
                [this->data setData:[NSData dataWithBytes:msg + 4 length:length - (pos + 4)]];
            }
        }
    }
    return YES;
...
- (NSString *)_version
{
    if(this->complete)
    {
        NSRange space = [this->statusText rangeOfCharacterFromSet:[NSCharacterSet whitespaceCharacterSet]];
        if(space.location != NSNotFound)
        {
            return [this->statusText substringToIndex:space.location];
        }
        else
        {
            return @"HTTP/1.1";
...
- (id) copyWithZone: (NSZone*)z
{
  ...
  if (NSShouldRetainWithZone(self, z) == YES)
      ...
      if (o != nil)
	{
      ...
      inst->data = [this->data mutableCopy];
	  if (this->headers == 0)
	    {
	      inst->headers = 0;
	    }
