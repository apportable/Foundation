...
#include <stdio.h>
#include <stdlib.h>
#if (DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_WINDOWS) && !APPORTABLE
#include <asl.h>
#else
...
#if DEPLOYMENT_TARGET_MACOSX && !APPORTABLE
...
#elif (DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_EMBEDDED_MINI) && !APPORTABLE
...
#elif APPORTABLE
    size_t outSize;
    int result = sysctlbyname("hw.availablecpu", &pcnt, &outSize, NULL, 0);
    if (result != 0)
    {
        pcnt = 0;
    }
...
#if 1 && (DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_EMBEDDED_MINI) && !APPORTABLE
    uid_t uid;
    gid_t gid;
...
static Boolean also_do_stderr() {
    ...
    return true;
#elif (DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED) && !APPORTABLE
