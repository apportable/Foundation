...
#if (DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED) && !APPORTABLE
...
#elif APPORTABLE
    if (strcmp(fileName, "/System/Library/CoreServices/CFCharacterSetBitmaps.bitmap") == 0)
    {
        extern size_t ___CFCharacterSetBitmapsSize;
        extern void *___CFCharacterSetBitmaps;
        *bytes = &___CFCharacterSetBitmaps;
        if (NULL != fileSize) *fileSize = ___CFCharacterSetBitmapsSize;
        return true;
    }
    else if (strcmp(fileName, "/System/Library/CoreServices/CFUnicodeData-L.mapping") == 0)
    {
        extern size_t ___CFUnicodeDataLSize;
        extern void * ___CFUnicodeDataL;
        *bytes = &___CFUnicodeDataL;
        if (NULL != fileSize) *fileSize = ___CFUnicodeDataLSize;
        return true;
    }
    else if (strcmp(fileName, "/System/Library/CoreServices/CFUnicodeData-B.mapping") == 0)
    {
        extern size_t ___CFUnicodeDataBSize;
        extern void * ___CFUnicodeDataB;
        *bytes = &___CFUnicodeDataB;
        if (NULL != fileSize) *fileSize = ___CFUnicodeDataBSize;
        return true;
    }
    else if (strcmp(fileName, "/System/Library/CoreServices/CFUniCharPropertyDatabase.data") == 0)
    {
        extern size_t ___CFUniCharPropertyDatabaseSize;
        extern void * ___CFUniCharPropertyDatabase;
        *bytes = &___CFUniCharPropertyDatabase;
        if (NULL != fileSize) *fileSize = ___CFUniCharPropertyDatabaseSize;
        return true;
    }
    else
    {
        DEBUG_LOG("%s NOT FOUND!", fileName);
    }
    return false;
#else
    struct stat statBuf;
