...
CF_PRIVATE void _CFRuntimeSetInstanceTypeIDAndIsa(CFTypeRef cf, CFTypeID newTypeID) {
    ...
    if (newTypeID < __CFRuntimeClassTableSize) {
        object_setClass(cf, (Class)__CFRuntimeObjCClassTable[newTypeID]);
    }
}
...
static inline Boolean isCFType(CFTypeRef obj) {
    CFTypeID typeID = __CFGenericTypeID_inline(obj);
    uintptr_t isa = ((CFRuntimeBase *)obj)->_cfisa;
    return isa == (uintptr_t)__CFConstantStringClassReferencePtr || (typeID < __CFRuntimeClassTableSize && isa == (uintptr_t)__CFRuntimeObjCClassTable[typeID]);
}
...
CFTypeRef _CFAutoreleasePoolAddObject(CFTypeRef cf) {
    return (CFTypeRef)_objc_rootAutorelease((id)cf);
}

void _CFAutoreleasePoolPop(void *ctx) {
...
extern void __CFAttributedStringInitialize(void);
extern void __CFURLInitialize(void);
extern void __CFKeyedArchiverUIDInitialize(void);
#if DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_EMBEDDED_MINI
...
void __CFInitialize(void) {
...
#if (DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_EMBEDDED_MINI) && !APPORTABLE
	UInt32 s, r;
	...
	pthread_atfork(__01121__, NULL, __01123__);
        ...
        Class cls = (Class)objc_getClass("__NSCFType");
        for (int idx = 0; idx < __CFRuntimeClassTableSize; idx++)
        {
            __CFRuntimeObjCClassTable[idx] = (uintptr_t)cls;
        }
...
#pragma clang diagnostic pop
        ...
        // CFBoolean and CFNumber initializers use bridged classes, so we bridge them early.
	...
	__CFMessagePortInitialize();
        ...
        __CFKeyedArchiverUIDInitialize();
        __CFDateInitialize();
        ...
        char *strtokSavePtr;
        CFIndex count = 0;
        CFStringRef *strings = (CFStringRef *)malloc(LANG_MAX * sizeof(CFStringRef));
        size_t size = LANG_MAX;
        ...
        for (char *lang = strtok_r(languages, ", ", &strtokSavePtr); lang; lang = strtok_r(NULL, ", ", &strtokSavePtr))
        {
            ...
            strings[count++] = CFStringCreateWithBytes(kCFAllocatorDefault, lang, strlen(lang), kCFStringEncodingUTF8, false);
        }
        CFArrayRef languageArray = CFArrayCreate(kCFAllocatorDefault, (const void **)strings, count, &kCFTypeArrayCallBacks);
        CFPreferencesSetAppValue(CFSTR("AppleLanguages"), languageArray, kCFPreferencesCurrentApplication);
        for (CFIndex idx = 0; idx < count; idx++)
        {
            CFRelease(strings[idx]);
        }
