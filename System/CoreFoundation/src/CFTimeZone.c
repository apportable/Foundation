...
CFTimeZoneRef CFTimeZoneCreateWithName(CFAllocatorRef allocator, CFStringRef name, Boolean tryAbbrev) {
    ...
    if (NULL == data) {
       ...
       if (NULL != tempURL) {
           ...
           {
                // No data files in /usr/share/zoneinfo - eg file://localhost/usr/share/zoneinfo/America/Los_Angeles
                // Get from data file
                extern const uint8_t *_TimeZoneDataGet(const char *key, CFIndex *len);

                CFIndex strLen = CFStringGetLength(tzName);
                const char *cTzName = CFStringGetCStringPtr(tzName, kCFStringEncodingASCII);
                if (NULL != cTzName)
                {
                    bytes = (void *)_TimeZoneDataGet(cTzName, &length);
                    if (NULL != bytes)
                    {
